  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto h-14 px-3 sm:px-6 flex items-center justify-between gap-2 sm:gap-4">
        {/* ── LEFT: Clean Brand Identity (Clicking returns to first screen) ── */}
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <button
            id="brand-logo-btn"
            type="button"
            onClick={handleBrandClick}
            className="flex items-center gap-2.5 cursor-pointer group text-left border-0 bg-transparent p-0"
            title="OSR Studio - Go to start screen"
          >
            {/* Minimalist Studio Aperture Badge */}
            <div className="relative flex items-center justify-center">
              <div className="relative w-8 h-8 rounded-xl bg-gradient-to-tr from-[#000000] via-zinc-800 to-zinc-950 dark:from-[#000000] dark:via-zinc-900 dark:to-zinc-800 p-[1.5px] shadow-xs flex items-center justify-center group-hover:scale-105 transition-transform border border-white/10">
                <div className="w-full h-full rounded-[10px] bg-[#000000] flex items-center justify-center overflow-hidden relative">
                  <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-tr from-[#8DB355] to-[#FFEA93] p-[1px] shadow-xs flex items-center justify-center">
            ...
            {/* Clean Modern Brand Name */}
            <span className="text-sm sm:text-base font-black tracking-tight text-slate-900 dark:text-zinc-100 font-sans group-hover:text-slate-700 dark:group-hover:text-white transition-colors">
              OSR Studio
            </span>
          </button>
        </div>

        {/* ── FLEX SPACER (Keeps Left and Right balanced on sides) ── */}
        <div className="flex-1" />

        {/* ── CENTER: Mathematically True Centered Mode Switcher or Live Recording Timer with Pause ── */}
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center pointer-events-none z-20">
          <div className="pointer-events-auto">
            {/* Live Recording Timer Pill & Nearby Single Pause Button */}
            {isRecording ? (
              <div className="flex items-center gap-2">
                <div
                  id="header-live-recording-badge"
                  className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#D90000]/10 dark:bg-[#D90000]/20 border border-[#D90000]/30 text-[#D90000] dark:text-red-400 font-bold text-xs sm:text-sm animate-in fade-in shadow-xs"
                >
                ...
                {/* Pause / Resume Button beside Timer ONLY */}
                {onTogglePause && (
                  <button
                    type="button"
                    id="header-pause-btn"
                    onClick={onTogglePause}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all cursor-pointer shadow-xs active:scale-95 ${
        ...
        {/* ── RIGHT: Library button, Theme Switcher & Primary Record/Pause/Stop Action ── */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-semibold rounded-xl border transition-all cursor-pointer active:scale-95 ${
              activeView === 'library'
          ...
          <button
            ...
            className="relative inline-flex items-center justify-between w-[52px] h-7 p-1 rounded-full bg-slate-200/90 dark:bg-zinc-800 border border-slate-300/80 dark:border-zinc-700/80 cursor-pointer transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 shadow-xs active:scale-95"
          >
            {/* Sun slot on left */}
          ...
          {/* Small, sleek Download Button in Top Header during Video Editing */}
          {activeView === 'studio' && recordingState === 'editing' && onOpenEditorExport && (
            <button
              id="nav-editor-download-button"
              type="button"
              onClick={onOpenEditorExport}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#D90000] hover:bg-[#b80000] active:scale-95 text-white font-bold text-xs shadow-md shadow-[#D90000]/25 transition-all cursor-pointer"
          ...
          {/* Primary Quick Record / Pause & Stop Action Buttons (ONLY when NOT in editing or review) */}
          {activeView === 'studio' &&
            hasSelectedInitialMode &&
            recordingState !== 'editing' &&
            recordingState !== 'review' &&
            ...
            <div className="shrink-0 flex items-center gap-1.5 sm:gap-2">
              {!isRecording ? (
                <button
                  ...
                  className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 rounded-full bg-[#D90000] hover:bg-[#b80000] active:scale-95 text-white font-bold text-xs sm:text-sm shadow-md shadow-[#D90000]/30 transition-all cursor-pointer"
                ...
                /* Stop Button ONLY (single Pause button is centered beside timer) */
                <button
                  id="top-stop-recording-button"
                  type="button"
                  disabled={isStoppingRecording}
                  onClick={onStopRecording}
                  className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-full ${
                    isStoppingRecording
                      ? 'bg-[#b80000] opacity-80 cursor-wait'
                      : 'bg-[#D90000] hover:bg-[#b80000] active:scale-95 cursor-pointer'
                  } text-white font-bold text-xs sm:text-sm shadow-lg shadow-[#D90000]/40 transition-all`}
