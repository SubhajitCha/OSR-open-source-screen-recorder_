...
export const RecordingsLibrary: React.FC<RecordingsLibraryProps> = ({
  ...
  return (
    <div id="recordings-library-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      {/* Header with Title & Storage Gauge styled with brand palette */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#121215] p-6 rounded-3xl border border-slate-200/90 dark:border-zinc-800 shadow-xs transition-colors">
        <div className="flex items-center gap-3.5">
          <div className="flex items-center justify-center w-11 h-11 rounded-2xl bg-[#D90000]/10 dark:bg-[#D90000]/15 text-[#D90000] border border-[#D90000]/20 shadow-xs">
        ...
        {/* Right Action & Storage quota card */}
        <div className="flex items-center flex-wrap gap-3">
          <div className="flex items-center gap-3.5 px-3.5 py-2 rounded-2xl bg-slate-50 dark:bg-zinc-900/80 border border-slate-200/80 dark:border-zinc-800 text-xs">
            ...
            <div className="w-20 bg-slate-200 dark:bg-zinc-800 rounded-full h-1.5 overflow-hidden">
              <div
                className="bg-[#D90000] h-full rounded-full transition-all duration-300"
                style={{ width: `${Math.min(100, Math.max(storageInfo.percentage, 5))}%` }}
              />
            </div>
      ...
      {allTags.length > 0 && (
        <div className="flex items-center gap-1.5 overflow-x-auto w-full pb-1">
          <button
            ...
            className={`px-3.5 py-1.5 text-xs rounded-full font-bold transition-colors cursor-pointer ${
              selectedTag === 'all'
                ? 'bg-[#000000] dark:bg-white text-white dark:text-black shadow-xs'
          ...
          {allTags.map((tag) => (
            <button
              ...
              className={`px-3.5 py-1.5 text-xs rounded-full font-semibold capitalize transition-colors cursor-pointer ${
                selectedTag === tag
                  ? 'bg-[#000000] dark:bg-white text-white dark:text-black shadow-xs'
                  : 'bg-white dark:bg-[#121215] text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-zinc-800 shadow-xs'
              }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecordings.map((rec) => (
            <div
              ...
              className="group flex flex-col rounded-3xl bg-white dark:bg-[#121215] border border-slate-200/90 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 transition-all duration-200 overflow-hidden shadow-xs hover:shadow-md"
            >
              {/* Thumbnail Container with Play Overlay */}
              <div
                onClick={() => setActivePlayback(rec)}
                className="relative aspect-video w-full bg-[#000000] cursor-pointer overflow-hidden"
              >
                ...
                {/* Duration Badge */}
                <div className="absolute bottom-2.5 right-2.5 px-2 py-0.5 rounded-full bg-black/80 backdrop-blur-sm text-[11px] font-mono font-bold text-white border border-white/10">
                  {formatDuration(rec.duration)}
                </div>

                {/* Mode icon badge */}
                <div className="absolute top-2.5 left-2.5 px-2.5 py-0.5 rounded-full bg-black/80 backdrop-blur-sm text-[10px] font-semibold text-white border border-white/10 capitalize flex items-center gap-1">
                  {rec.mode === 'screen_cam' ? (
                    <Layers01Icon className="w-3 h-3 text-[#FFEA93]" />
                  ) : rec.mode === 'cam_only' ? (
                    <Camera01Icon className="w-3 h-3 text-[#FF6666]" />
                  ) : rec.mode === 'audio_only' ? (
                    <RadioIcon className="w-3 h-3 text-[#8DB355]" />
                  ) : (
                    <Film01Icon className="w-3 h-3 text-slate-300" />
                  )}
                ...
                <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-[#D90000] text-white shadow-lg shadow-[#D90000]/40 group-hover:scale-110 transition-transform">
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                  ...
                  {editingId === rec.id ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="flex-1 px-2.5 py-1 text-xs bg-slate-50 dark:bg-zinc-900 border border-[#D90000] rounded-lg text-slate-900 dark:text-zinc-100"
                        autoFocus
                      />
                      <button
                        onClick={() => handleSaveEdit(rec.id)}
                        className="p-1 text-[#8DB355] hover:bg-slate-100 dark:hover:bg-zinc-800 rounded cursor-pointer"
                      >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <h3
                        onClick={() => setActivePlayback(rec)}
                        className="text-sm font-bold text-slate-900 dark:text-zinc-100 hover:text-[#D90000] dark:hover:text-[#FF6666] cursor-pointer line-clamp-1 transition-colors"
                      >
                        {rec.title}
                      </h3>
                      <button
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-zinc-800/80">
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setActivePlayback(rec)}
                      className="flex items-center gap-1 text-xs font-bold text-[#D90000] dark:text-[#FF6666] hover:underline cursor-pointer transition-colors"
                    >
                    ...
                    {onSelectRecordingForEdit && (
                      <button
                        ...
                        className="flex items-center gap-1 text-xs font-bold text-slate-900 dark:text-zinc-100 hover:bg-slate-100 dark:hover:bg-zinc-800 cursor-pointer transition-colors bg-slate-100/80 dark:bg-zinc-900 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-zinc-800"
                        title="Open in Studio Editor with auto-zoom & background effects"
                      >
                        <PencilEdit02Icon className="w-3 h-3 text-[#D90000]" />
                        <span>Edit</span>
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 dark:text-zinc-500 hover:text-[#D90000] rounded-full hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors cursor-pointer"
      ...
      {deleteTarget && (
        ...
        >
          <div className="w-full max-w-md bg-white dark:bg-[#121215] rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-2xl p-6 space-y-4 text-slate-900 dark:text-zinc-100 animate-in zoom-in-95 duration-200">
            <div className="flex items-start gap-3.5">
              <div className="w-10 h-10 rounded-2xl bg-[#D90000]/10 text-[#D90000] flex items-center justify-center shrink-0 border border-[#D90000]/20">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
                  Are you sure you want to delete <span className="font-semibold text-slate-900 dark:text-white">"{deleteTarget.title}"</span> from your offline storage? This action cannot be undone.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100 dark:border-zinc-800">
              ...
              <button
                ...
                className="px-5 py-2 text-xs font-bold text-white bg-[#D90000] hover:bg-[#b80000] rounded-full shadow-md shadow-[#D90000]/25 transition-all cursor-pointer active:scale-95 disabled:opacity-50"
              >
        ...
        >
          <div className="w-full max-w-md bg-white dark:bg-[#121215] rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-2xl p-6 space-y-4 text-slate-900 dark:text-zinc-100 animate-in zoom-in-95 duration-200">
            <div className="flex items-start gap-3.5">
              ...
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Delete All Recordings?</h3>
                <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
      ...
      {activePlayback && (
        ...
        >
          <div className="flex flex-col w-full max-w-4xl max-h-[90vh] bg-white dark:bg-[#121215] border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden text-slate-900 dark:text-zinc-100">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#121215]">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{activePlayback.title}</h3>
            ...
            <div className="flex-1 p-6 flex flex-col items-center justify-center bg-[#000000]">
              {playbackUrl && (
                <video
                  src={playbackUrl}
                  controls
              ...
              <div className="flex items-center gap-2">
                {onSelectRecordingForEdit && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 rounded-full border border-slate-200 dark:border-zinc-700 shadow-xs cursor-pointer transition-colors"
                  >
                    <PencilEdit02Icon className="w-4 h-4 text-[#D90000]" />
